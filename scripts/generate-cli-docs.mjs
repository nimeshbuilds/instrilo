...
const lines = ['# Instrilo CLI reference', '', 'Generated from the CLI itself. Regenerate with `npm run docs:cli` after changing commands or manual topics.', '', 'Install the built app and CLI from the [GitHub release](https://github.com/nimeshbuilds/instrilo/releases) using the [installation guide](INSTALLATION.md). Run `instrilo web enable` to open the local app. Or build from source with `npm ci --ignore-scripts` and `npm run build`, then run `node dist/cli.js web enable`; replace `instrilo` with `node dist/cli.js` in the examples from that repository root. `npm link` is optional. `PROJECT`, `RUN_ID`, `HASH`, and uppercase example values are placeholders.', '', `This reference contains ${commands.length} command entries (including groups and the root) and ${Object.keys(manuals).length} offline manual topics.`, '', '```sh', 'instrilo help --all', 'instrilo help approvals approve', 'instrilo explain --list', 'instrilo explain --search JWT', 'instrilo help --json', '```', '', '## Command index', '', '| Command | Purpose |', '| --- | --- |'];
