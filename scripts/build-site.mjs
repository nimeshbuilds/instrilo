...
function home(scenarios, results, verification) {
  ...
  <section class="setup-section wrap" id="setup" aria-labelledby="setup-title"><div class="setup-intro"><p class="eyebrow">Ready-built release · ${releaseVersion}</p><h2 id="setup-title">One install.<br>App and CLI.</h2><p>Open a local browser workbench or work entirely from your terminal. Both come in the same release package.</p><div class="prereq-inline"><strong>Before you start</strong><p>Node.js 22 or newer and npm.<br>macOS, Linux, or Windows with WSL.<br>Python agents also need Python 3.11–3.13 and uv.</p><div class="resource-links"><a href="https://nodejs.org/en/download">Get Node.js ${arrow}</a><a href="https://docs.astral.sh/uv/getting-started/installation/">Get uv ${arrow}</a><a href="https://learn.microsoft.com/en-us/windows/wsl/install">Set up WSL ${arrow}</a></div></div></div><div class="setup-code">${codeBlock(install, 'Install the release · Bash / zsh')}<p class="code-caption">Expect version ${releaseVersion}, then open the private local URL printed in your terminal. Keep the terminal running; stop the app with Ctrl+C. npm downloads runtime dependencies.</p><div class="resource-links"><a href="${releasePage}">Release assets &amp; checksums ${arrow}</a><a href="${repository}/blob/main/docs/INSTALLATION.md">Full installation guide ${arrow}</a></div><details class="setup-detail"><summary>Permission error or command not found?</summary><p>Use a directory you own. This installs without administrator access and makes the command available in this terminal.</p>${codeBlock(`npm install --global --prefix "$HOME/.local/instrilo" ${releasePackage}\nexport PATH="$HOME/.local/instrilo/bin:$PATH"\ninstrilo --version`, 'Install in your user directory')}<p>The <a href="${repository}/blob/main/docs/INSTALLATION.md#install-without-administrator-access">installation guide</a> explains how to keep PATH in new terminals, upgrade, and uninstall. The short alias <code>in</code> has every CLI feature; in Bash, use <code>command in</code> because <code>in</code> is a reserved word.</p></details><p class="code-caption">The app runs locally in your browser. This package requires Node.js; it is not a native desktop executable. GitHub Pages hosts these guides.</p></div></section>
  <section class="setup-section source-setup-section wrap" id="source-setup" aria-labelledby="source-setup-title"><div class="setup-intro"><p class="eyebrow">For the walkthroughs below</p><h2 id="source-setup-title">Explore ten<br>tested scenarios.</h2><p>The exact walkthroughs use repository fixtures and source-relative commands. Set up a separate source checkout to follow them unchanged. Normal app and CLI use only needs the release above.</p><p>First guide: no model account needed.</p></div><div class="setup-code">${codeBlock(sourceInstall, 'Prepare the walkthroughs · Git required')}<p class="code-caption">Run every guide from this repository folder, using its <code>node dist/cli.js</code> commands. Python guides also need Python and uv.</p><div class="resource-links"><a href="quickstarts/${scenarios[0].id}/index.html">Build your first agent ${chevron}</a><a href="${repository}/blob/main/docs/INSTALLATION.md#source-installation-for-the-ten-walkthroughs">Source setup details ${arrow}</a></div></div></section>
