...
const { version: releaseVersion } = JSON.parse(await readFile(join(root, 'package.json'), 'utf8'));
if (typeof releaseVersion !== 'string' || !/^\d+\.\d+\.\d+$/.test(releaseVersion)) throw new Error('The site needs a stable release version from package.json.');
const releasePage = `${repository}/releases/tag/v${releaseVersion}`;
const releasePackage = `${repository}/releases/download/v${releaseVersion}/nimeshbuilds-instrilo-${releaseVersion}.tgz`;
...
function footer(prefix) {
  return `<footer class="site-footer"><div class="footer-inner"><div><a class="brand footer-brand" href="${prefix}index.html"><img src="${prefix}assets/mark.svg" width="42" height="42" alt=""><span>Instrilo</span></a><p>Build with intent. Ship with evidence.</p></div><div class="footer-links"><a href="${releasePage}">Download the release ${arrow}</a><a href="${repository}/blob/main/docs/INSTALLATION.md">Installation guide ${arrow}</a><a href="${repository}">Source code ${arrow}</a><a href="${repository}/blob/main/CONTRIBUTING.md">Contribute ${arrow}</a><a href="${repository}/issues">Get help ${arrow}</a><a href="${repository}/blob/main/LICENSE">Apache-2.0 ${arrow}</a></div><p class="footer-note">An open-source project by <a href="https://github.com/nimeshbuilds">nimeshbuilds</a>.<br>Built for code you can own, inspect, and change.</p></div></footer>`;
}
...
function home(scenarios, results, verification) {
  ...
  const install = `npm install --global ${releasePackage}\ninstrilo web enable`;
  const sourceInstall = 'git clone https://github.com/nimeshbuilds/instrilo.git\ncd instrilo\nnpm ci --ignore-scripts\nnpm run build\nnpm link\ninstrilo web enable';
  ...
  <section class="installation-section wrap" id="setup" aria-labelledby="setup-title"><div class="section-heading"><div><p class="eyebrow">Your workbench · ${releaseVersion}</p><h2 id="setup-title">Choose your way in.</h2></div><p>Download the built release or build from source. Both give you the same CLI and local web app.</p></div><div class="installation-prereqs"><strong>Before you start</strong><p>Node.js 22 or newer and npm on macOS, Linux, or Windows with WSL. Python agents also need Python 3.11–3.13 and uv.</p><div class="resource-links"><a href="https://nodejs.org/en/download">Get Node.js ${arrow}</a><a href="https://docs.astral.sh/uv/getting-started/installation/">Get uv ${arrow}</a><a href="https://learn.microsoft.com/en-us/windows/wsl/install">Set up WSL ${arrow}</a></div></div><div class="installation-choices"><article class="installation-card" id="release-setup"><p class="eyebrow">Ready to use</p><h3>Download the release</h3><p>The compiled app and CLI, in one package. Install it without cloning or compiling.</p>${codeBlock(install, 'Install and open · Bash / zsh')}<p class="code-caption">npm fetches runtime dependencies. The final command opens the local app in your browser.</p><div class="resource-links"><a href="${releasePackage}">Download package ${arrow}</a><a href="${releasePage}">Release notes &amp; checksums ${arrow}</a></div><details class="setup-detail"><summary>Permission error or command not found?</summary><p>Install in a directory you own, then add its commands to PATH for this terminal.</p>${codeBlock(`npm install --global --prefix "$HOME/.local/instrilo" ${releasePackage}\nexport PATH="$HOME/.local/instrilo/bin:$PATH"\ninstrilo web enable`, 'Install in your user directory')}<p>The <a href="${repository}/blob/main/docs/INSTALLATION.md#install-without-administrator-access">installation guide</a> covers keeping PATH in new terminals, upgrading, and uninstalling.</p></details></article><article class="installation-card" id="source-setup"><p class="eyebrow">Make it your own</p><h3>Build from source</h3><p>For contributing to Instrilo or changing its code. Requires Git; all quickstarts also work with the built release.</p>${codeBlock(sourceInstall, 'Build and open · Bash / zsh')}<p class="code-caption"><code>npm link</code> makes the built <code>instrilo</code> command available from any folder. All guides use that same command. Source setup details include a user-owned prefix if linking reports a permission error.</p><div class="resource-links"><a href="https://git-scm.com/downloads">Get Git ${arrow}</a><a href="${repository}/blob/main/docs/INSTALLATION.md#build-from-source-instead">Source setup details ${arrow}</a></div></article></div><div class="web-launch-note" id="open-app"><div><p class="eyebrow">From here, just one command</p><h3>Your browser opens.<br>Your projects stay with you.</h3></div><div>${codeBlock('instrilo web enable', 'Open the app anytime')}<p>Projects live in <code>~/Instrilo/projects</code>. Keep the terminal running and stop with Ctrl+C. Run the command again to reopen your projects. No model account is needed to start.</p><p>Choose another folder with <code>--workspace ./my-projects</code>, a fixed port with <code>--port 4317</code>, or open the printed URL yourself with <code>--no-open</code>.</p><div class="resource-links"><a href="${repository}/blob/main/docs/INSTALLATION.md#open-customize-and-stop-the-app">App startup &amp; troubleshooting ${arrow}</a><a href="${repository}/blob/main/docs/INSTALLATION.md#start-from-the-cli">Prefer the CLI? Create your first agent ${chevron}</a></div></div></div><p class="installation-note">The app runs on your computer and requires Node.js. GitHub Pages hosts these guides. Keep the printed local session URL private. The short alias <code>in</code> has every feature; in Bash use <code>command in web enable</code>.</p></section>
  <section class="quickstarts-section wrap" id="quickstarts" aria-labelledby="quickstarts-title"><div class="section-heading"><div><p class="eyebrow">The field guide</p><h2 id="quickstarts-title">What will you build first?</h2></div><p>Ten practical paths, from your first local agent to a complete build with graph, judge, release evidence, and cloud artifacts. Every guide uses the installed CLI and bundled tutorial inputs. No source checkout is needed. First guide: no model account needed.</p></div><div class="guide-tools" hidden><label class="search-label" for="guide-search"><span class="sr-only">Search quickstarts</span><span aria-hidden="true">⌕</span><input id="guide-search" type="search" placeholder="Find a workflow…" autocomplete="off"></label><div class="category-filters" aria-label="Filter quickstarts by category"><button type="button" data-filter="" aria-pressed="true">All guides</button>${categories.map(category => `<button type="button" data-filter="${escape(category)}" aria-pressed="false">${escape(category)}</button>`).join('')}</div><p class="guide-count" id="guide-count" role="status" aria-live="polite">${scenarios.length} guides</p></div><div class="scenario-grid">${scenarios.map(item => card(item, results.get(item.id))).join('')}</div><div class="empty-state" hidden><h3>No matching guides</h3><p>Try a broader search or select another category.</p><button class="button button-primary" type="button" data-reset-search>Show all guides</button></div></section>
  ...
  <section class="next-section wrap"><div><p class="eyebrow">Room to make it yours</p><h2>Start with a guide.<br>Keep going with the CLI.</h2></div><div><p>Every command has help. The built-in manual explains complete workflows, even when you are offline.</p>${codeBlock('instrilo explain --list\ninstrilo help --all', 'Explore the built-in manual')}<div class="resource-links"><a href="${repository}/blob/main/docs/CLI.md">Browse the full CLI reference ${arrow}</a><a href="${repository}/blob/main/docs/ROADMAP.md">See the roadmap ${arrow}</a></div></div></section>
  </main>` });
}
...
export async function buildSite(outputDirectory = resolve(root, '_site'), options = {}) {
  ...
  const publicVerification = {
    ...
    installation: typeof verification?.installation === 'string' ? verification.installation : null,
    package: typeof verification?.package === 'string' ? verification.package : null,
    version: typeof verification?.version === 'string' ? verification.version : null,
    packageSha256: /^[0-9a-f]{64}$/.test(verification?.packageSha256 ?? '') ? verification.packageSha256 : null,
