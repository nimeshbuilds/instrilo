...
import { deploymentGuide, deploymentPlatform } from './deployment-platforms.js';
...
function validateGeneration(spec: ProjectSpec): void {
  ...
  if (spec.delivery.target === 'azure-container-apps' && (spec.name.length >= 32 || spec.name.length < 2 || spec.name.includes('--') || spec.name.endsWith('-'))) throw new Error('Azure Container Apps CLI requires a name of 2–31 characters (fewer than 32 characters), ending in a letter or digit, without consecutive hyphens');
  if (spec.delivery.target === 'cloud-run' && (spec.name.length >= 50 || spec.name.endsWith('-'))) throw new Error('Cloud Run requires a service name shorter than 50 characters, ending in a letter or digit');
  if (spec.delivery.target === 'aws-agentcore' && (spec.name.length < 2 || spec.name.endsWith('-'))) throw new Error('The AgentCore ECR repository name requires at least 2 characters and must end in a letter or digit');
...
export function generateArtifacts(spec: ProjectSpec, guidance: GuidanceReport): Artifact[] {
  ...
  const output: Artifact[] = [
    ...
    artifact(`session.${ext}`, template(`session.${ext}.tpl`)),
    ...
    artifact('DEPLOYMENT.md', deploymentGuide(spec, envNames)),
    artifact('SECURITY.md', securityGuide()),
    artifact('deployment-plan.json', json({ schemaVersion: 1, ...deploymentPlatform(spec.delivery.target), status: 'generated-not-deployed', region: spec.delivery.region ?? null, auth: spec.security.inbound.mode, secretEnvironmentNames: envNames, testing: { sourceProjectRequired: true, planCommand: 'instrilo deployment test PROJECT --engine docker', executeCommand: 'instrilo deployment test PROJECT --engine docker --execute', reportsCommand: 'instrilo deployment reports PROJECT', cleanupPlanCommand: 'instrilo deployment cleanup RUN_ID PROJECT', cleanupExecuteCommand: 'instrilo deployment cleanup RUN_ID PROJECT --execute', limitations: ['Mocked model/identity tests do not validate cloud IAM, registry permissions, model entitlement, billing or availability.', 'Cross-architecture execution requires native hardware or supported emulation.'] } })),
  ];
...
function containerArtifacts(spec: ProjectSpec): Artifact[] {
  ...
  const architecture = deploymentPlatform(spec.delivery.target).architecture;
  const dockerfile = py
...
function awsArtifacts(spec: ProjectSpec, envNames: string[]): Artifact[] {
  ...
  const policy = { Version: '2012-10-17', Statement: [
    ...
    { Effect: 'Allow', Action: ['logs:CreateLogGroup', 'logs:DescribeLogStreams'], Resource: 'arn:aws:logs:REPLACE_REGION:REPLACE_ACCOUNT_ID:log-group:/aws/bedrock-agentcore/runtimes/*' },
    { Effect: 'Allow', Action: ['logs:CreateLogStream', 'logs:PutLogEvents'], Resource: 'arn:aws:logs:REPLACE_REGION:REPLACE_ACCOUNT_ID:log-group:/aws/bedrock-agentcore/runtimes/*:log-stream:*' },
...
command -v gcloud >/dev/null; command -v jq >/dev/null; command -v docker >/dev/null
docker info >/dev/null
docker buildx inspect --bootstrap >/dev/null
gcloud artifacts repositories describe agents --project "$GCP_PROJECT" --location "$GCP_REGION" >/dev/null
...
args=(run deploy ${spec.name} --project "$GCP_PROJECT" --region "$GCP_REGION" --image "$image" --service-account "$GCP_SERVICE_ACCOUNT" --port 8080 --no-allow-unauthenticated --concurrency 1 --timeout ${Math.min(3600, Math.max(30, Math.ceil(spec.agent.limits.timeoutMs / 1000) + 10))} --set-env-vars HOST=0.0.0.0)
...
function azureArtifacts(spec: ProjectSpec, envNames: string[]): Artifact[] {
  const config = { location: spec.delivery.region ?? 'REPLACE_REGION', name: spec.name, type: 'Microsoft.App/containerApps', properties: { environmentId: 'REPLACE_ENVIRONMENT_RESOURCE_ID', configuration: { ingress: { external: true, targetPort: 8080, transport: 'auto', allowInsecure: false }, registries: [{ server: 'REPLACE_REGISTRY.azurecr.io', identity: 'REPLACE_IDENTITY_RESOURCE_ID' }], secrets: envNames.map((n, i) => ({ name: 'secret-' + (i + 1), keyVaultUrl: 'REPLACE_KEY_VAULT_SECRET_URL', identity: 'REPLACE_IDENTITY_RESOURCE_ID' })) }, template: { containers: [{ name: 'agent', image: 'REPLACE_IMAGE', env: [{ name: 'HOST', value: '0.0.0.0' }, { name: 'PORT', value: '8080' }, ...envNames.map((n, i) => ({ name: n, secretRef: 'secret-' + (i + 1) }))], resources: { cpu: 1, memory: '2Gi' }, probes: [{ type: 'Readiness', httpGet: { path: '/ping', port: 8080 }, initialDelaySeconds: 5, periodSeconds: 10 }, { type: 'Liveness', httpGet: { path: '/ping', port: 8080 }, initialDelaySeconds: 15, periodSeconds: 20 }] }], scale: { minReplicas: 0, maxReplicas: 3 } } }, identity: { type: 'UserAssigned', userAssignedIdentities: { REPLACE_IDENTITY_RESOURCE_ID: {} } } };
  const script = `#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/.."
: "\${AZURE_RESOURCE_GROUP:?Set AZURE_RESOURCE_GROUP}"; : "\${AZURE_REGISTRY:?Set AZURE_REGISTRY}"
: "\${AZURE_REGION:?Set AZURE_REGION}"; : "\${AZURE_ENVIRONMENT_ID:?Set AZURE_ENVIRONMENT_ID}"
: "\${AZURE_IDENTITY_ID:?Set the pre-authorized user-assigned AZURE_IDENTITY_ID}"
command -v az >/dev/null; command -v jq >/dev/null; command -v docker >/dev/null
docker info >/dev/null
docker buildx inspect --bootstrap >/dev/null
az account show >/dev/null
registry=$(az acr show --name "$AZURE_REGISTRY" --query loginServer --output tsv)
image="$registry/${spec.name}:$(date +%Y%m%d%H%M%S)"
mkdir -p .deployment
jq --arg image "$image" --arg registry "$registry" --arg identity "$AZURE_IDENTITY_ID" --arg environment "$AZURE_ENVIRONMENT_ID" --arg location "$AZURE_REGION" '.location=$location | .properties.environmentId=$environment | .identity={type:"UserAssigned",userAssignedIdentities:{($identity):{}}} | .properties.template.containers[0].image=$image | .properties.configuration.registries[0]={server:$registry,identity:$identity} | .properties.configuration.secrets |= map(.identity=$identity)' deploy/azure-container-app.json > .deployment/azure-container-app.yaml
if jq -e 'any(.. | strings; contains("REPLACE"))' .deployment/azure-container-app.yaml >/dev/null; then echo "Complete Key Vault references in deploy/azure-container-app.json first" >&2; exit 1; fi
az acr login --name "$AZURE_REGISTRY"
docker buildx build --platform linux/amd64 -t "$image" --push .
az containerapp create --name ${spec.name} --resource-group "$AZURE_RESOURCE_GROUP" --yaml .deployment/azure-container-app.yaml
`;
  return [artifact('deploy/azure-container-app.json', json(config)), artifact('deploy/azure-container-app.yaml', '# Review copy; deploy/azure.sh renders from the canonical JSON file.\n' + yaml(config)), artifact('deploy/azure.sh', script, true), artifact('deploy/azure-notes.md', '# Azure adapter\n\nFollow ../DEPLOYMENT.md for prerequisites, identity grants, registry, Key Vault, invocation and cleanup. The scaffold uses a pre-provisioned user-assigned managed identity for the first private image pull and secret resolution. Edit azure-container-app.json; the adjacent YAML is a review copy. The script renders JSON (valid YAML) into .deployment/azure-container-app.yaml after substituting explicit environment IDs. It refuses unresolved secret placeholders before image push.\n')];
}
