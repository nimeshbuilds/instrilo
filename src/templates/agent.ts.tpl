...
import { DurablePause, finishSession } from './session.js';
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  catch (error) { if (error instanceof DurablePause) console.log(JSON.stringify({ output: '', status: 'paused', pause: error.detail })); else { console.error(JSON.stringify({ error: error instanceof Error ? error.message : String(error) })); process.exitCode = 1; } }
