...
import { createProject, buildProject, refineProject, manifestName, safeChild } from './workbench.js';
import { providerCapabilities, diagnoseConnection } from './providers.js';
import { evaluateProject, runProject, saveReport, compareReports, sanitizeError, prepareProject, assertBuildCurrent } from './execution.js';
import { startServer } from './server.js';
import type { GuidanceAnswers, Language, Framework, Target } from './types.js';

const program = new Command().name('instrilo').version('0.1.0').description('Instrilo — turn product guidance into agents, evaluations and deployment artifacts.');
...
guidance.command('create <directory>').description('Ask the ten product questions and write a reviewable guidance document.').option('--answers <file>', 'JSON answers for noninteractive creation').action(async (directory, options) => {
  ...
  if (options.answers) answers = JSON.parse(await readFile(resolve(options.answers), 'utf8'));
  else {
...
program.command('plan').description('Ask the builder to synthesize instructions and identify unresolved questions.').argument('[project]', 'project directory or manifest', '.').option('--apply', 'save the proposed description and instructions').option('--interview', 'answer the builder’s specific follow-up questions and refine again').action(async (project, options) => {
  ...
  const path = manifest(project), spec = await loadSpec(path), guidanceDir = resolve(dirname(path), spec.guidanceDir);
  let proposed = await refineProject(spec, await inspectGuidance(guidanceDir));
  if (options.interview && proposed.questions.length) {
    ...
    proposed = await refineProject(spec, await inspectGuidance(guidanceDir));
  }
  print(proposed);
  if (options.apply) { spec.description = proposed.description; spec.agent.systemPrompt = proposed.systemPrompt; await saveSpec(path, spec); console.log('Saved proposed instructions. Review unresolved questions.'); }
});

program.command('build').description('Generate framework code, host packages, tests and target deployment artifacts.').argument('[project]', 'project directory or manifest', '.').option('-o, --output <directory>', 'generated project destination').option('--overwrite', 'replace existing generated files').action(async (project, options) => print(await buildProject(manifest(project), options.output, options.overwrite)));
program.command('prepare').description('Install generated runtime dependencies using npm or uv.').argument('[project]', 'project directory or manifest', '.').action(async project => { const path = manifest(project); print(await prepareProject(await loadSpec(path), join(dirname(path), 'generated'))); });
program.command('run').description('Run the actual generated agent locally. Provider usage may be billed.').argument('[project]', 'project directory or manifest', '.').requiredOption('-i, --input <text>', 'agent input').action(async (project, options) => { const path = manifest(project); const spec = await loadSpec(path); print(await runProject(spec, join(dirname(path), 'generated'), options.input)); });
program.command('eval').description('Run deterministic checks and the selected judge against the generated agent.').argument('[project]', 'project directory or manifest', '.').option('--split <split>', 'development, holdout, or all', 'all').option('--output <path>', 'report JSON destination').action(async (project, options) => {
  ...
  const report = await evaluateProject(spec, join(dirname(path), 'generated'), cases, { split: options.split, onProgress: (done, total) => console.error(`Evaluated ${done}/${total}`) });
  const reportPath = options.output ? resolve(options.output) : join(dirname(path), 'reports', `${Date.now()}-${report.id}.json`); await saveReport(reportPath, report); print({ ...report, reportPath });
...
program.command('deploy').description('Show generated delivery instructions; --execute runs the target script you configured.').argument('[project]', 'project directory or manifest', '.').option('--execute', 'execute generated deployment script (requires configured cloud CLI and variables)').action(async (project, options) => {
  ...
  const code = await new Promise<number>((resolveCode, reject) => {
    const child = spawn('bash', [scriptPath], { cwd: outputDir, env: process.env, stdio: 'inherit', shell: false });
    child.once('error', reject); child.once('close', exitCode => resolveCode(exitCode ?? 1));
  });
